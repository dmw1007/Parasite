...
/// 这两个回调用于检测是否已敲入气泡或条目。
/// 如果是这种情况，[ArticlePage]将被推入[Navigator]堆栈。
...
///
/// 可从[RenderBox]访问此窗口小部件的字段，以便可以将其与当前状态对齐。
...
///
/// 该对象的核心方法是[paint（）]：这是所有元素实际绘制到屏幕上的地方。
class TimelineRenderObject extends RenderBox {
  ...
  void paint(PaintingContext context, Offset offset) {
      ...
      for (TimelineAsset asset in timeline.renderAssets) {
        if (asset.opacity > 0) {
          ...
          if (asset is TimelineImage) {
            ...
            /// 2.将[canvas]移动到正确的位置，以使小部件的位置根据其偏移量，大小和对齐位置居中对齐.
            canvas.translate(
                renderOffset.dx +
    ...
    /// 当用户按下时间轴右上角的心形按钮时，会出现左侧的装订线，以便您可以快速访问喜欢的元素。
    ///
    ...
    if (_favorites != null && _favorites.length > 0 && favoritesGutter > 0.0) {
      ...
      /// 如果装订线中有两个或多个收藏夹，请显示一条连接两个圆圈的线，并将两个收藏夹之间的时间标记为气泡。
      ///
  ...
  void drawItems(PaintingContext context, Offset offset,
    ...
    for (TimelineEntry item in entries) {
      ...
      /// 根据气泡的宽度和高度获取气泡的路径，进行绘制，然后在顶部添加标签。
      Path bubble =
  ...
  /// 给定宽度和高度，为时间轴上位于事件标签后面的气泡设计路径，然后将其返回。
