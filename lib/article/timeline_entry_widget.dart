...
/// 此小部件呈现单个[TimelineEntry]。它依赖于[LeafRenderObjectWidget]
/// 因此它可以实现自定义[RenderObject]并进行相应的更新。
class TimelineEntryWidget extends LeafRenderObjectWidget {
  /// 仅在需要时才使用标志来为小部件设置动画。
  final bool isActive;
  final TimelineEntry timelineEntry;
  /// 如果此小部件还具有自定义控制器，则[interactOffset]
  ///参数可用于检测运动效果并相应地更改[FlareActor]。
...
/// 
/// 特别是，这意味着要覆盖[paint（）]和[hitTestSelf（）]方法，以将放置小部件的位置加载的Flare / Nima [FlutterActor]呈现出来。
class VignetteRenderObject extends RenderBox {
  ...
  /// 此重写的方法是我们可以实现自定义逻辑的位置，
  ///布置[FlutterActor]，然后将其绘制到[canvas]。
  @override
  void paint(PaintingContext context, Offset offset) {
    ...
    if (asset is TimelineImage) {
      ...
      /// 如果我们拥有[TimelineNima]资源，请对其进行正确设置并进行绘制。
      ///
      /// 1.计算当前对象的边界。
      ///首次加载资产时，已经设置了Axis-Aligned Bounding Box（AABB）。
      ///我们依靠此AABB来执行屏幕空间计算。
      ...
      /// 此小部件始终设置为使用[BoxFit.contain]。
      ///但是可以根据任何人的需求自定义此行为。
      ///以下开关/案例包含Flutter固有的所有其他选择。
      switch (fit) {
        case BoxFit.fill:
      ...
      /// 4.将画布移动到上面计算出的正确[_nimaActor]位置。
