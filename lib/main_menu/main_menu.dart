...
///
/// 该小部件位于页面顶部的搜索栏上，其中三个卡片部分用于访问时间轴上的主要事件，并在底部的三个链接中提供了快速访问收藏夹，共享菜单和关于页面的链接。
...
class _MainMenuWidgetState extends State<MainMenuWidget> {
  ...
  initState() {
    ...
    /// 该资产提供了卡片的所有必要信息，
    /// 例如标签，背景颜色，背景Flare动画素材资源，
    /// 对于扩展卡中的每个元素，[timeline]上的相对位置.
  ...
  /// [WillPopScope]小部件包装了菜单，因此在关闭整个应用程序之前，
  /// 搜索将首先弹出。否则，应用程序将照常进行。
  ...
  Widget build(BuildContext context) {
    ...
    if (_isSearching) {
      ...
      tail
        ...
        ..add(FlatButton(
            onPressed: () => Share.share("这是一个关于寄生虫的科普APP" +
                (Platform.isAndroid
                    ? ""
                    : "")),
            color: Colors.transparent,
